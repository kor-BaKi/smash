...
- 상세 스펙은 `docs/05-API명세.md`, 핵심 로직은 `docs/04-핵심로직.md` 참조.

### 7.1 커밋 정책

- 커밋 단위 = "컴파일되고 의미가 있는 단위"가 끝났을 때. 엔티티→Repository→Service→Controller→테스트
  흐름 전체가 끝날 때까지 묶어두지 않는다. (예: 1단계 인증은 [엔티티+Repository] / [JWT 유틸] /
  [Security 필터] / [Auth API 동작 확인] 각각 1커밋 정도가 기준)
