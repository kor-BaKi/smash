...
class StatisticsApi {
  ...
  Future<List<MonthlyFulfillment>> getFulfillmentTrend({
    ...
    try {
      final response = await _dio.get(
        '/admin/statistics/fulfillment-trend',
        queryParameters: {
          'groupId': ?groupId,
          'fromYear': fromYear,
          'fromMonth': fromMonth,
  ...
  Future<List<GroupComparisonItem>> getGroupComparison(
    int year,
    int month,
  ) async {
    try {
      final response = await _dio.get(
        '/admin/statistics/group-comparison',
        queryParameters: {'year': year, 'month': month},
      );
      final data = response.data['data'] as List;
      return data
          .map(
            (e) => GroupComparisonItem.fromJson(
              e as Map<String, dynamic>,
            ),
          )
          .toList();
  ...
  Future<List<MonthlyCarryoverCount>> getCarryoverTrend({
    ...
    try {
      final response = await _dio.get(
        '/admin/statistics/carryover-trend',
        queryParameters: {
          'fromYear': fromYear,
          'fromMonth': fromMonth,
          'toYear': toYear,
      ...
      return data
          .map(
            (e) => MonthlyCarryoverCount.fromJson(
              e as Map<String, dynamic>,
            ),
          )
          .toList();
